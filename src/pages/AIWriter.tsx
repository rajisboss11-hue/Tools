...
export default function AIWriter() {
  ...
  const generateDraft = async () => {
    ...
    try {
      // Using the built-in Gemini API key from the environment
      const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Write a comprehensive, SEO-optimized blog post draft about: "${topic}". Include an engaging title, introduction, 3-4 main sections with headings, and a conclusion. Format it in Markdown.`,
      });
      
      setDraft(response.text || 'Failed to generate content.');
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="border-b pb-4">
        ...
        <p className="text-gray-600 mt-2">Generate SEO-optimized blog post drafts instantly using Gemini AI.</p>
      </div>
